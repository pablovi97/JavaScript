...
export class PokemonsListComponent implements OnInit, OnDestroy {
  ...
  onAdd(pok: Pokemons) {
    if (pok) {
      //Verificamos si tiene un id o nombre repetido antes de añadirlo
      for (const val of this.pokemons) {
        if (val.id == pok.id ) {
          this.log('No se puede añadir un pokemon con un id creado...');
          return;
        }else if(val.name == pok.name){
          this.log('No se puede añadir un pokemon con un name creado...');
          return;
        }
      }
